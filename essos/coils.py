...
def _initialize_currents_scale(currents, currents_scale):
    ...
    if currents_scale is None:
        mean_absolute = jnp.mean(jnp.abs(currents))
        return jnp.where(mean_absolute > 0, mean_absolute, jnp.ones_like(mean_absolute))
...
class Coils:
    ...
    def base_currents(self):
        """Physical currents of independent coils before symmetry expansion."""
        return self.dofs_currents_raw
    ...
    def from_json(cls, filename: str):
        ...
        if "dofs_curves_raw" in data:
            ...
            # Fork files explicitly record normalized and physical currents;
            # original unscaled legacy files have no currents_scale metadata.
            import numpy as np
            if "base_currents" in data:
                currents_raw = jnp.asarray(data["base_currents"])
                if data.get("currents_scale") is not None and "dofs_currents" in data:
                    reconstructed = np.asarray(data["dofs_currents"]) * float(data["currents_scale"])
                    if reconstructed.shape != currents_raw.shape or not np.allclose(
                        reconstructed, np.asarray(currents_raw), rtol=1e-12, atol=1e-12
                    ):
                        raise ValueError("inconsistent current representations in coil JSON")
            elif data.get("currents_scale") is not None:
                currents_raw = jnp.asarray(data["dofs_currents"]) * data["currents_scale"]
            else:
                currents_raw = jnp.asarray(data["dofs_currents"])
        
        # Create Coils object with raw currents
        return cls(curves, currents_raw, currents_scale=data.get("currents_scale", None))
    
    def _tree_flatten(self):
        # Current scales can be arrays or tracers when coils are built inside
        # a transformed design function. Keep them out of static metadata so
        # nested JIT calls never compare traced arrays as Python booleans.
        children = (self.curves, self.dofs_currents, self.currents_scale)
...
# Legacy fork entry points retained alongside the released class methods.
def Coils_from_json(filename):
    """Load legacy XYZ coil JSON using :meth:`Coils.from_json`."""
    return Coils.from_json(filename)


def Coils_from_simsopt(simsopt_coils, nfp=1, stellsym=True, **kwargs):
    """Import SIMSOPT coils using :meth:`Coils.from_simsopt`."""
    return Coils.from_simsopt(simsopt_coils, nfp=nfp, stellsym=stellsym, **kwargs)
